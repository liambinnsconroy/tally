...
function SetupScreen({state,dispatch}){
  ...
  return(<div style={{padding:"20px 20px 100px"}}>
    ...
    {gameKey==="custom"&&<div style={{...S.limeCard,padding:16,marginBottom:24}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16}}>{[{v:"standard",l:"📊 Standard"},{v:"independent",l:"⭐ Independent"}].map(({v,l})=><button key={v} onClick={()=>setScoringType(v)} style={{flex:1,padding:"12px 0",fontSize:13,fontWeight:700,background:scoringType===v?C.tomato:C.white,color:scoringType===v?C.white:C.ink,border:"2px solid "+C.ink,borderRadius:10,cursor:"pointer"}}>{l}</button>)}</div>
